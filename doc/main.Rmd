...
# show candidates chosen for the typo: "acress" 
...
* Prior probability $Pr(c)$:
    ...
    + Use ELE estimation: $Pr(c)=(freq(c) + 0.5)/(N + V/2)$ deal with the problem that MLE performs poorly when observed freqency of a word is zero, wherr\e
...
#load results from error detection part
load("/Users/yujiewang/Google Drive (yw3285@columbia.edu)/12345/Fall2018-Project4-sec1-grp8/output/total_detect_output.RData")
load("/Users/yujiewang/Google Drive (yw3285@columbia.edu)/12345/Fall2018-Project4-sec1-grp8/output/total_tesseract_clean_output.RData")
load("/Users/yujiewang/Google Drive (yw3285@columbia.edu)/12345/Fall2018-Project4-sec1-grp8/output/total_ground_truth_txt_100.RData")
load("/Users/yujiewang/Google Drive (yw3285@columbia.edu)/12345/Fall2018-Project4-sec1-grp8/output/total_tesseract_txt_100.RData")


#clean words in test group
test_group_clean <- total_tesseract_clean_output[-train_index]

#typo list to be corrected in the test group
test_group <- total_detect_output[-train_index]
typo_list <- list()
...
for(i in 1:20) {
    ...
    for (j in current_file){
      if (grepl("[[:digit:]]",j) | grepl("^[a-zA-Z]$", j) | grepl("ﬂu", j)){
        ...
        correction <- c(correction, perform_correction(j))
      }
...
ground_truth_test <- total_ground_truth_txt_100[-train_index]
#tesseract file without punctutations in the test group
OCR_test <- total_tesseract_txt_100[-train_index]
...
- Word level: The precision and recall of the original Tessearct OCR outputs are 61.51105% and 64.57310%. With post-processing, both precision and recall increased around 3%. Adopting the idea of SVM garbage detection and probability scoring with contextual constraints can somewhat improve the Tesseract OCR output in terms of correcting words.

- Character level: All lowercase and uppercase letters are considered. The original Tessearct OCR outputs has precision 90.39345% and recall 91.00901%. After post-processing, the results are quite similar to the original ones, with the precision score and the recall both increasing around 0.6%.
