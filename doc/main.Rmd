...
## Test the functions built for probability scoring
...
```{r}
priors <- prior_prob(train_truth_text)

#calculate the prior probability of the typo "acress" 
priors[acress_candidate]
```
...
## Performing corrections on test files
...
elerecall <- rep(NA, 20)
for (i in 1:20){
recall[i] <- (length(vecsets::vintersect(ground_truth_test_vec[[i]], combine_list[[i]]))/length(ground_truth_test_vec[[i]])) * weights_recall[i]
}


elerecall <- sum(recall)

#wordwise precision ele

eleprecision <- rep(NA, 20)
for (i in 1:20){
precision[i] <- (length(vecsets::vintersect(ground_truth_test_vec[[i]], combine_list[[i]]))/length(OCR_test_vec[[i]])) * weights_prec[i]
}

eleprecision <- sum(precision)
...
length(vecsets::vintersect(tes_char, ground_char))/ length(ground_char)


# character precision for tesseract
length(vecsets::vintersect(tes_char, ground_char))/ length(tes_char)


# character recall for ele
correction_char <- unlist(str_split(unlist(combine_list), ""))
length(vecsets::vintersect(correction_char, ground_char))/ length(ground_char)


# character precision for ele
length(vecsets::vintersect(correction_char, ground_char))/ length(tes_char)
```
